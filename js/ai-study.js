...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  if (downloadPdfBtn) {
    downloadPdfBtn.addEventListener('click', () => {
      try {
        const docTitle = studyData.documentTitle || 'EasyScan Notes';
        ...
        if (jsPDFClass) {
          ...
          doc.text('EasyScan — Study Notes', margin, y);
          y += 22;
...
`;
  ...
  if (shareBtn) {
    shareBtn.addEventListener('click', async () => {
      const shareText = `📚 ${studyData.documentTitle || 'Study Notes'}\n\n${derivedSummary}\n\nGenerated by EasyScan`;
      if (navigator.share) {
        try {
          await navigator.share({
            title: studyData.documentTitle || 'EasyScan Notes',
            text: shareText,
