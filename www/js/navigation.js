...
const StudyGenNav = (() => {
  ...
  function initHardwareBackButton() {
    if (window._hasInitBackButton) return;
    window._hasInitBackButton = true;

    try {
      const cap = window.Capacitor;
      if (cap && (typeof cap.isNativePlatform === 'function' ? cap.isNativePlatform() : (cap.getPlatform && cap.getPlatform() !== 'web'))) {
        const App = cap.Plugins && cap.Plugins.App;
        if (App && typeof App.addListener === 'function') {
          App.addListener('backButton', () => {
            // 1. Close modal overlays if open
            const overlay = document.getElementById('studygen-confirm-overlay') || document.getElementById('studygenActionSheet');
            if (overlay) {
              overlay.remove();
              return;
            }

            // 2. If on root pages (home, index, login), exit app cleanly
            const currentPage = getCurrentPage();
            if (currentPage === 'home' || currentPage === 'index' || currentPage === 'login') {
              App.exitApp();
            } else {
              goBack('home.html');
            }
      ...
      console.warn('Hardware back button setup skipped:', e);
    }
  }
