...
const ApiClient = (() => {
  ...
  // Production Render Backend API for Android APK / hosted environments
  const PRODUCTION_API_URL = 'https://docscanner-project.onrender.com/api';

  // Detect native Capacitor Android environment
  const isCapacitor = typeof window !== 'undefined' && (
    (window.Capacitor && typeof window.Capacitor.isNativePlatform === 'function' && window.Capacitor.isNativePlatform()) ||
    window.location.protocol === 'capacitor:' ||
    (typeof window.Capacitor !== 'undefined' && isLocalhost)
  );

  const BASE_URL = isCapacitor
    ? PRODUCTION_API_URL
    : (isFileProtocol || isDevPort || (isLocalhost && port === '5000')) 
